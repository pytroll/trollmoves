...
class TestSSHMovers(unittest.TestCase):
    ...
    def test_scp_copy_uses_the_default_scpclient_timeout(self, mock_scp_client, mock_sshclient):
        """Check that the scp client waits the default time for a response when nothing is configured."""
        from trollmoves.movers import DEFAULT_SCPCLIENT_TIMEOUT, ScpMover

        scp_mover = ScpMover(self.origin, self.destination_no_port, attrs=self._attrs_empty)
        scp_mover.copy()

        assert mock_scp_client.call_args.kwargs["socket_timeout"] == DEFAULT_SCPCLIENT_TIMEOUT
    ...
    def test_scp_copy_response_timeout_suggests_a_longer_timeout(self, mock_scp_client, mock_sshclient):
        """Check that a timed-out transfer points the user at the setting that would give it more time."""
        from scp import SCPException

        from trollmoves.movers import SCP_RESPONSE_TIMEOUT_MESSAGE, ScpMover

        mock_scp_client.return_value.put.side_effect = SCPException(SCP_RESPONSE_TIMEOUT_MESSAGE)
